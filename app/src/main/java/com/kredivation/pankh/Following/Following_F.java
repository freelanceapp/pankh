package com.kredivation.pankh.Following;
...
public class Following_F extends Fragment {
    ...
    private void Call_Api_For_get_Allfollowing() {
        ...
        if (Utility.isOnline(getActivity())) {
            ...
            serviceCaller.CallCommanServiceMethod(Variables.get_followingsNew, parameters, "Call_Api_For_get_Allfollowing", new IAsyncWorkCompletedCallback() {
                @Override
                public void onDone(String result, boolean isComplete) {
                    Parse_following_data(result);
                }
            });
        } else {
    ...
    private void Call_Api_For_get_Allfan() {
       ...
       /* ApiRequest.Call_Api(context, Variables.get_followers, parameters, new Callback() {
        ...
        if (Utility.isOnline(getActivity())) {
            ServiceCaller serviceCaller = new ServiceCaller(getActivity());
            serviceCaller.CallCommanServiceMethod(Variables.get_followersNew, parameters, "Call_Api_For_get_Allfan", new IAsyncWorkCompletedCallback() {
                @Override
                public void onDone(String result, boolean isComplete) {
                    Parse_fans_data(result);
                }
            });
        }else {
            Toast.makeText(context, Variables.OFFLINE_MESSAGE, Toast.LENGTH_SHORT).show();
